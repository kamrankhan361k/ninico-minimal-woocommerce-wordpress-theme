...
function ninico_breadcrumb_func() {
    global $post, $title;  
    $breadcrumb_class = '';
    $breadcrumb_show = 1;
    ...
    if ( is_front_page() && is_home() ) {
        $title = get_theme_mod('breadcrumb_blog_title', __('Blog','ninico'));
    ...
    elseif ( is_single() && 'product' == get_post_type() ) {
        $title = get_theme_mod( 'breadcrumb_product_details', __( 'Shop', 'ninico' ) );
    } 
    elseif ( is_single() && 'courses' == get_post_type() ) {
      $title = esc_html__( 'Course Details', 'ninico' );
    } 
    elseif ( 'courses' == get_post_type() ) {
      $title = esc_html__( 'Courses', 'ninico' );
    } 
    elseif ( is_search() ) {
        $title = esc_html__( 'Search Results for : ', 'ninico' ) . get_search_query();
    } 
    elseif ( is_404() ) {
        $title = esc_html__( 'Page not Found', 'ninico' );
    } 
    elseif ( function_exists( 'is_woocommerce' ) && is_woocommerce() ) {
        $title = get_theme_mod( 'breadcrumb_shop', __( 'Shop', 'ninico' ) );
    } 
    elseif ( is_archive() ) {
    ...
    $bg_img_color = get_theme_mod( 'ninico_breadcrumb_bg_color', '#F0F1F3');
    $ninico_breadcrumb_shape_switch = get_theme_mod( 'ninico_breadcrumb_shape_switch', true );
...
add_action( 'ninico_header_style', 'ninico_breadcrumb_func', 10 );

// ninico_search_form
function ninico_search_form() {
    ?>
        ...
        <div class="header-search-bar-5">
            <form method="get" action="<?php print esc_url( home_url( '/shop' ) );?>" >
                <div class="search-info-5 p-relative">
                    ...
                    <input type="text" name="s" value="<?php print esc_attr( get_search_query() )?>" placeholder="<?php print esc_attr__( 'Search products...', 'ninico' );?>">
                </div>
            </form>
        </div>
    </div>
...
// ninico_search_form_2
function ninico_search_form_2() {
    ?>
...
// ninico_search_form_3
function ninico_search_form_3() {
    ?>
...
// ninico_search_form_4
function ninico_search_form_4() {
    ?>
...
// ninico_offcanvas_search
function ninico_offcanvas_search() {
    ?>
    <form method="get" action="<?php print esc_url( home_url( '/shop' ) );?>">
        <input type="text" name="s" value="<?php print esc_attr( get_search_query() )?>" placeholder="<?php print esc_attr__( 'Search products...', 'ninico' );?>">
